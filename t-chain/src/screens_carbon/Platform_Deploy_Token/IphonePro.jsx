...
async function fetchTokenList (sessionId, address){
  ...
  const token_list_fetch_template = {
    "jsonrpc": "3.0",
    "method": "chain_carbon",
     "params":[`opcode=carbon&subcode=showPengindToken&address=${platform_address}&op=0`, "encryp=none"],
        "id": `${sessionId}`};
...
export const PlatformDeployToken = () => {
  ...
  const handleClick = (token_name, targetAddress, sessionId, setIsLoading, setShouldRefetch) => {
    ...
    const current_add_manager_template = {
      "jsonrpc": "3.0",
      "method": "chain_carbon",
       "params":[`opcode=carbon&subcode=deployToken&address=${platform_address}&addr1=${targetAddress}&name=${token_name}&type=0`, "encryp=none"],
        "id": `${sessionId}`};
  ...
  return (
    <div>
      ...
      <div className="div-2">
        ...
        <Sbumit className="sbumit-deploy-token" textKey="deploy_token" onClick={() => {
          navigate('/CarbonA/CarbonADeployToken');}
          } />
