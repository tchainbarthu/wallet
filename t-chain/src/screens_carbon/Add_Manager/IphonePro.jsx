...
const handleClick = (manager_name, targetAddress, sessionId, setIsLoading, navigate) => {
  ...
  const current_add_manager_template = {
    "jsonrpc": "3.0",
    "method": "chain_carbon",
     "params":[`opcode=carbon&subcode=addManager&address=${platform_address}&addr1=${targetAddress}&name=${manager_name}`, "encryp=none"],
      "id": `${sessionId}`};
  ...
  return (
    <div>
      ...
      <div className="div-2">
        ...
        <Sbumit className="sbumit-instance" textKey="add" onClick={() => {handleClick(managerName, targetAddress, sessionId, setIsLoading, navigate)}} />
        <Sbumit className="sbumit-instance-cancel" textKey="cancel" onClick={() => {navigate('/CarbonA/PlatformAddManager')}} />
